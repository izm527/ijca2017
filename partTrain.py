...
def trainModelRg2(train_data, train_label, test_data, test_label):
    ...
    model.add(avg)
    c1 = Conv2D(filters=12, kernel_size=(2,3),  strides=(2,2), padding='SAME', input_shape = shape(train_data[0]))
    model.add(c1)
    model.add(Activation('relu'))
    model.add(Flatten())
    model.add(Dense(8, activation='relu'))
    model.add(Dense(1))
    sgd = SGD (lr=0.00000002, decay=3e-4, momentum=0.8, nesterov=False)
    model.compile(loss='mean_squared_error', optimizer=sgd)
...
def trainModelLin3(train_data, train_label, test_data, test_label):
    ...
    sgd = SGD (lr=0.00000005, decay=8e-4, momentum=0.8, nesterov=False)
    model.compile(loss='mean_squared_error', optimizer=sgd)
    model.fit(train_data, train_label, batch_size = 10, epochs =200, validation_data=(test_data, test_label))
    return model
...
score = 0
modelFeatureFile = open("model/modelFeat.txt1", 'w')
kfoldFile = open("model/kfold.txt", 'r')
while score <= 4:
...
#    for train, test in kf.split(all_data):
    for cnt in range(4):
        train = kfoldFile.readline()
        train = train[0:-2]
        train = train.split(' ')
        test = kfoldFile.readline()
        test = test[0:-2]
        test = test.split(' ')
        for cc in range(len(train)):
            train[cc] = int(train[cc])
...
#///////////////////////////////////////////////////////////////////////////////////////////
        ...
        for i in range(len(prd)):
            ...
            res[1].append(val)
            err += (abs(val-test_label[i])/(val+test_label[i]))
        print "-----------------------------------model 2's err = ", err/len(prd)
...
#            res[2].append(val)
#            err += (abs(val-test_label[i])/(val+test_label[i]))
#        print "-----------------------------------model 3's err = ", err/len(prd)
#
#        indx += 1
