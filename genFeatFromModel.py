...
if T == 1:
    ...
    start = 372 + day - percase   # start from 0
    end = 448 +day- percase
else:
# final attempt
    percase = 1
    start = 373 + day - percase   # start from 0
    end = 449 + day - percase 
...
def main(start, end, percase):
    ...
    #    for shp in shops_sel:
    #        print payCntMat[shp[0]-1][start:end]
    ...
    #///////////////////////////////train sets//////////////////////////////////
        ...
        for shp in shff_Mat:
            ...
            print payCntMat[shp[0]][start+j:end+j], payCntMat[shp[0]][end+j]
        ...
        res0 = zeros(len(all_data))
        res1 = zeros(len(all_data))
        res2 = zeros(len(all_data))
        for k in range(foldnum):
            # result of the model0 : 80 days predictions
            model0 = load_model("model/md-score"+str(score)+'.'+str(k))
            prd = model0.predict(all_data,  verbose=1)
            for l in range(len(prd)):
                res0[l] += prd[l][0]
            print '\n', k, 'fold model 0'
            ...
            model1 = load_model("model/md-avg"+str(score)+'.'+str(k))
            ...
            model2 = load_model("model/md-7days"+str(score)+'.'+str(k))
        ...
        # create the last 3 month's average pays
        res0 /= foldnum
        res1 /= foldnum
        res2 /= foldnum
        ...
        print "score:"+str(score)+' model1 err:'+str(err)
        err = sum(abs(res2-all_label)/(res2+all_label)) / len(all_label)
        print "score:"+str(score)+' model2 err:'+str(err)
    
        for i in range(len(shff_Mat)):
            all_shp.append(shff_Mat[i][0])
            line = ""
    ...
    if T == 1:
        ...
        print "create the feature/train2/feature.txt"
    else:
        secondTrainFile = open("feature/attempt/test.txt", 'w')
        print "create the feature/attempt/test.txt"
    
    for line in Train:
        secondTrainFile.write(line)
    
    secondTrainFile.close()
    return [all_shp, all_data]

#return the predict data's shops' index to match the predict results
shp = main(start, end, percase)
shp = shp[0]
